...
func (vm *signInMailer) SendNewSignInMail(email string, device *session_entities.Device) error {
	return vm.mailer.SendMail(email, "Новый вход в аккаунт", vm.bodyFromTemplate(device))
}

func (vm *signInMailer) bodyFromTemplate(device *session_entities.Device) string {
